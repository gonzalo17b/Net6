...
app.MapGet("/settings/initial", (IOptions<Settings> options) => new ExternalSettings(options.Value));
app.MapGet("/settings/current", (IOptionsSnapshot<Settings> options) => new ExternalSettings(options.Value));
